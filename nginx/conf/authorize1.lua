...
local action,path,user,ip,uri,result,url,base64user,users,headers,permissions,file1,str,strjson,jsonStr,numbers,requesturl --日志动作、请求路径、用户名、请求地址、请求操作、返>回结果、请求链接、base用户名密码、解密用户名密码、请求头、权限校验、io流、权限json、转换json、发送admin的json、判断是否成功、请求日志url
local res = {} --用于装去重后的权限
local flags = {} --去重数组
...
local _ , _ , txt = string.find(url,'/webhdfs/v1'.."(.-)"..'?')
...
local _ , _ , txt2 = string.find(url,'op='.."(.-)"..'$')
